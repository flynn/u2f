...
func register(t webauthn.Token, host string) error {
	c := &http.Client{}
	...
	webauthnResp, err := t.Register(host, respData.Data.PublicKey)
	if err != nil {
		return err
	}
...
func authenticate(t webauthn.Token, host, session string) error {
	c := &http.Client{}
	reqBody := bytes.NewBuffer([]byte(`{"userVerification":"preferred"}`))
	...
	webauthnResp, err := t.Authenticate(host, respData.Data.PublicKey)
	if err != nil {
		panic(err)
	}
